...
export async function exportCsv(kind:"orders"|"expenses"|"summary",range:DateRange){ const result=await dialog.showSaveDialog({defaultPath:`gruhswad-${kind}-${range.from}-${range.to}.csv`,filters:[{name:"CSV",extensions:["csv"]}]}); if(result.canceled||!result.filePath)return null; const summary=dashboard(range);const rows=kind==="orders"?listUnifiedOrders({kind:"all",range,search:"",source:""}).map((row)=>({record_type:row.kind,reference:row.reference,customer:row.customerName,phone:row.phone,email:row.email,service_date:row.serviceDate,fulfilment:row.fulfilment,source:row.sourceName,status:row.status,items:row.items,total:row.total,payment_status:row.paymentStatus,notes:row.notes,created_at:row.createdAt})):kind==="expenses"?listExpenses(range):[{from:range.from,to:range.to,revenue:summary.revenue,expenses:summary.expenses,profit:summary.profit,orderCount:summary.orderCount,outstanding:summary.outstanding,averageOrder:summary.averageOrder}]; writeFileSync(result.filePath,csv(rows as Record<string,unknown>[])); return result.filePath; }
