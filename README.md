...
Current release: **0.4.0**. The application includes Overview, cloud and local service Orders, Tiffin plans, Inventory, Recipes, Customers, Catalog, menu publishing, Operations, Expenses, Reports, Sync Centre, and Settings. The WhatsApp Business inbox backend remains present, but its renderer shortcut is currently disabled and marked **Coming soon**.
...
- **Orders** separates read-only website handoff state from the internal operational workflow. Online status changes show as pending until Sync Centre publishes them through the restricted Neon function. Confirmed, Ready, and Cancelled orders expose a trusted one-click WhatsApp action; the local audit records only that WhatsApp was opened.
- **Enquiry conversion** creates one reviewed, confirmed local Party/Bulk order or Tiffin plan and records only its reference in Neon. Party/Bulk selected-line quantities default to the larger numeric value from guest count and quantity expectations. Tiffin conversions restore current requirement fields and use the larger people/quantity expectation as their effective service count. Pending Neon updates remain retryable without duplicating the local service.
- **Inventory, Recipes, and Tiffin plans** track weighted-average stock, linked purchase expenses, reservations/consumption, recipe versions and optional YouTube preparation videos, plus explicit weekly/monthly Tiffin cycles. Manual plans capture people and optional quantity expectations; cycle quantities use the larger value before applying scheduled days and meal slots. New plans may select or create a customer, and plan financials aggregate the ledgers of their generated cycles.
...
Artifacts are written to `release/`, including `Gruhswad-Admin-Setup-0.4.0.exe`, its block map, `latest.yml`, and `win-unpacked`. Windows SmartScreen may warn because version 0.4.0 is unsigned.

Pushing a `v*` tag runs `.github/workflows/release.yml`; merging a pull request alone intentionally does not publish a release. The tag must exactly match the package version (`0.4.0` uses `v0.4.0`) and point to the reviewed commit on `master`.

Packaging explicitly runs `pnpm native:electron` before Electron Builder so `better-sqlite3` targets Electron's Node ABI. Electron Builder's implicit native rebuild is disabled; do not remove the explicit rebuild from local packaging or either platform workflow.
...
$env:APP_ENV="prod"
pnpm.cmd build
pnpm.cmd release:verify-tag -- v0.4.0
...
The published 0.4.0 release is available at <https://github.com/amol-pandhare/gruhswad-admin-desktop/releases/tag/v0.4.0>.
