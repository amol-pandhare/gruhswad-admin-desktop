...
function repairRuntimeSettings(){const rows=sqlite.prepare("SELECT settings_key,payload FROM app_settings").all() as {settings_key:string;payload:string}[],config=runtimeConfigFromRows(rows),upsert=sqlite.prepare("INSERT INTO app_settings(settings_key,payload,updated_at) VALUES(?,?,COALESCE((SELECT updated_at FROM app_settings WHERE settings_key=?),?)) ON CONFLICT(settings_key) DO UPDATE SET payload=excluded.payload");for(const [key,payload] of [["site",config.site],["operations",config.operations],["announcement",config.announcement],["service_area",config.serviceArea],["ordering_platforms",config.orderingPlatforms],["public_location",config.publicLocation]] as const)upsert.run(key,JSON.stringify(payload),key,new Date().toISOString());}
...
export function saveRuntimeConfig(config:RuntimeConfig){const now=new Date().toISOString(),upsert=sqlite.prepare("INSERT INTO app_settings(settings_key,payload,updated_at) VALUES(?,?,?) ON CONFLICT(settings_key) DO UPDATE SET payload=excluded.payload,updated_at=excluded.updated_at");sqlite.transaction(()=>{for(const [key,payload] of [["site",config.site],["operations",config.operations],["announcement",config.announcement],["service_area",config.serviceArea],["ordering_platforms",config.orderingPlatforms],["public_location",config.publicLocation]] as const){upsert.run(key,JSON.stringify(payload),now);markDirty("app_setting",key,payload);}})();}
